...
ConsoleOutput("Kullanıcı arayüzü başlatıldı")
class Map(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        for uav in uavList:
            if uav.info["Bağlı"] == "Evet":
                qp.drawEllipse(
                    400+(uav.info["X"]*100)-5, 400-(uav.info["Y"]*100)-5, 10, 10)
...
class GroupInfos(QTableWidget):
    ...
    def __init__(self):
        ...
        self.labels = ["Daire", "Grup No", "Dronelar",
                       "Formasyon", "Formasyon Merkezi"]
    ...
    def StartCircle(self, group):
        ConsoleOutput("Çembere başlanıyor...")
    ...
    def StopCircle(self, group):
        ConsoleOutput("Çember durduruluyor...")
        for tmp_group in groups.groups[group]:
    ...
    def UpdateLabels(self):
        ...
        for group in groups.groups:
            ...
            table_formation = QTableWidgetItem(
                str(groups.formation_info[i-1][0]))
...
class Form_Connect(QFormLayout):
    ...
    def AddSwarmURI(self, uri):
        ...
        self.swarm_count += 1
        ConsoleOutput("Sürüye yeni bir eleman eklendi")
    ...
    def RemoveSwarm(self):
        ...
        self.swarm_count -= 1
        ConsoleOutput("Sürüden bir eleman çıkartıldı")
    ...
    def RemoveSwarmAll(self):
        while self.Lines:
            ...
            self.removeRow(self.swarm_count-1)
            self.swarm_count -= 1
        ConsoleOutput("Sürünün bütün elemanları çıkartıldı.")
    ...
    def LoadURI(self):
        if os.path.exists("config/URIs.txt") and os.stat("config/URIs.txt").st_size:
            ...
            msg.exec_()
            ConsoleOutput("Hata, konfigürasyon dosyası yok ya da boş!")
...
class Form_SetFormation(QFormLayout):
    ...
    def submit(self):
        ...
        groups.SetFormationInfos(int(self.group.text()), self.cb.currentText(), "X : " + str(
            center_x) + "  Y : " + str(center_y) + "  Z : " + str(center_z), center_x, center_y, center_z)

        #################################
        formation = Formation()
        formation.Cokgen(max(formation_side, len(group)), distance,
                         formation_side, Pose(center_x, center_y, center_z))
        poses = formation.GetSides()
        ConsoleOutput(max(formation_side, len(group) ) + " kenarlı, " + "(" + center_x + ", " + center_y + ", " + center_z + ") merkezli bir çokgen oluşturuluyor...")
        #################################
        ...
        for uav in uavList:
            if uav.info["Bağlı"] == "Evet" and uav.info["Grup"] == int(self.group.text()):
                ...
                for pose in poses:
                    dist.append(uav.distance_to_dest(
        ...
        for index in indexes:
            ...
            uavList[uav_id].SetDistanceToCenter(
                [center_x, center_y, center_z], uavList[uav_id].GetDest())
...
class Form_Hareket(QFormLayout):
    ...
    def submit(self):
        ...
        if group in groups.formation_info and groups.formation_info[group][0] != "Yok":
            ConsoleOutput("Sürü merkezi (" + old_center[0] + ", " + old_center[1] + ", " + old_center[2] + ")" + "noktasından (" + center_x + ", " + center_y + ", " + center_z + ") noktasına geçiriliyor...")
            old_center = groups.GetCenter(group)
            new_center = [center_x, center_y, center_z]
            groups.SetFormationInfos(group, groups.formation_info[group][0], "X : " + str(
                center_x) + "  Y : " + str(center_y) + "  Z : " + str(center_z), center_x, center_y, center_z)
    ...
    def PopUp(self):
        ...
        msg.exec_()
        ConsoleOutput("Hata! Formasyon oluşturulamadı ya da istenen grup mevcut değil!")
...
class Form_Assemble(QFormLayout):
    def __init__(self, dialog):
        ...
        ConsoleOutput("Gruplarda Değişiklik. Eski Grup: " + self.first + " Yeni Grup: " + self.second )
...
class MapLayout(QHBoxLayout):
    ...
    def calculate(self):
        try:
            ...
            ConsoleOutput("Hata. Form_Assembly::calculate fonksiyonuna sayı veriniz.")
    ...
    def update_pos(self):
        ...
        for i in self.pos:
            ...
            label = QLabel("Pos " + str(self.pos.index(i)) +
                           " :     X  :" + str(x) + "     Y :" + str(y))
    ...
    def submit(self):
        ...
        for uav in uavList:
            if uav.info["Bağlı"] == "Evet" and uav.info["Grup"] == int(self.group.text()):
                ...
                for pose in self.calculatedposes:
                    dist.append(uav.distance_to_dest(
                        [float(pose[0]), float(pose[1]), float(self.height.text())]))
                initial_cost.append(dist)
        ...
        for index in indexes:
            uavList[uav_ids[index[0]]].SetDest(
                self.calculatedposes[index[1]][0], self.calculatedposes[index[1]][1], self.height.text())
    ...
    def PopUp(self):
        msg = QMessageBox()
        msg.setWindowTitle("Dikkat")
        msg.setText(str(len(groups.groups[int(self.group.text(
        ))])) + " Pozisyon Gerekli " + str(len(self.calculatedposes)) + " Pozisyon Verildi")
        msg.exec_()
        ConsoleOutput(str(len(groups.groups[int(self.group.text(
        ))])) + " Pozisyon Gerekli " + str(len(self.calculatedposes)) + " Pozisyon Verildi")
...
class TrajectoryMap(QHBoxLayout):
    ...
    def calculate(self):
        try:
            ...
            self.uzaklık.setText("Lütfen Sayı Giriniz")
            ConsoleOutput("Hata. Trajectory_Map::calculate fonksiyonuna sayı veriniz.")
    ...
    def PopUp(self):
        ...
        msg.setText("Grup formasyon oluşturmadı veya öyle bir grup yok.")
        msg.exec_()
        ConsoleOutput("Hata! Grup formasyonu oluşturamadı veya istenilen grup mevcut değil.")
...
class buttons(QGridLayout):
    ...
    def __init__(self):
        ...
        buttons = [["Bağlantıyı kur", 0, 0], ["Bağlantıyı kes", 0, 1], ["Drone Kaldırma/İndirme", 0, 2], ["Yeni Formasyon", 1, 0],  ["Formasyon ", 1, 1], ["Hareket ", 1, 2],
                   ["Trajectory", 2, 0], ["Sürü Ayırma", 2, 1], ["Sürü Birleştirme", 2, 2], ["İHA Ayırma / Ekleme", 3, 0], ["Uçuş Planlaması Yap", 3, 1], ["Planlanan Uçuşa Başla", 3, 2]]
...
class Window(QWidget):
    ...
    def showTimeSim(self):
        ...
        for group in groups.groups:
            ...
            for uav in group:
                tmp_group.append(
                    [uavList[uav].info["X"], uavList[uav].info["Y"], uavList[uav].info["Z"]])
            tmp_groups.append(tmp_group)

        self.simulation.sc.CalculateAllLines(
            tmp_groups, self.simulation.simulation_aktif)
